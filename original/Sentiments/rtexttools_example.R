...
#NYTimes <- NYTimes[sample(1:200,size=200,replace=FALSE),]
data <-coded
...
#matrix <- create_matrix(cbind(NYTimes["Title"],NYTimes["Subject"]), language="english", removeNumbers=TRUE, stemWords=TRUE)

matrix <- create_matrix(data$text, language="english", removeNumbers=TRUE, stemWords=TRUE)
...
container <- create_container(matrix,data$sentiment,trainSize=1:800,virgin=FALSE)
...
# CREATE A TERM-DOCUMENT MATRIX THAT REPRESENTS WORD FREQUENCIES IN EACH DOCUMENT
# WE WILL TRAIN ON THE Title and Subject COLUMNS
#new_matrix <- create_matrix(cbind(NYTimes["Title"],NYTimes["Subject"]), language="english", removeNumbers=TRUE, stemWords=TRUE, originalMatrix=matrix)
new_matrix <- create_matrix(data$text, language="english", removeNumbers=TRUE, stemWords=TRUE, originalMatrix=matrix)
...
container <- create_container(new_matrix,coded$sentiment,testSize=1:100,virgin=FALSE)
